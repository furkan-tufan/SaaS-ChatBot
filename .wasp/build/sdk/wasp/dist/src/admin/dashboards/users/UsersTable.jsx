...
function AdminSwitch({ id, isAdmin }) {
    return (<SwitcherOne isOn={isAdmin} onChange={(value) => updateIsUserAdminById({ id: id, isAdmin: value })}/>);
}
const UsersTable = () => {
    const [currentPage, setCurrentPage] = useState(1);
    const [emailFilter, setEmailFilter] = useState(undefined);
    const [isAdminFilter, setIsAdminFilter] = useState(undefined);
    const [subscriptionStatusFilter, setSubcriptionStatusFilter] = useState([]);
    ...
    return (<div className='flex flex-col gap-4'>
      <div className='rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark'>
        <div className='flex-col flex items-start justify-between p-6 gap-3 w-full bg-gray-100/40 dark:bg-gray-700/50'>
          ...
          <div className='flex items-center justify-between gap-3 w-full px-2'>
            <div className='relative flex items-center gap-3 '>
              ...
              <input type='text' id='email-filter' placeholder='dude@example.com' onChange={(e) => {
            ...
            setEmailFilter(value === '' ? undefined : value);
              ...
              <div className='flex-grow relative z-20 rounded border border-stroke pr-8 outline-none bg-white transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input'>
                <div className='flex items-center'>
                  {subscriptionStatusFilter.length > 0 ? (subscriptionStatusFilter.map((opt) => (<span key={opt} className='z-30 flex items-center my-1 mx-2 py-1 px-2 outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary'>
                ...
                setSubcriptionStatusFilter((prevValue) => {
                    return prevValue?.filter((val) => val !== opt);
                      ...
                      </span>))) : (<span className='bg-white text-gray-500 py-2 px-5 outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary'>
        ...
        }} name='status-filter' id='status-filter' className='absolute top-0 left-0 z-20 h-full w-full bg-white opacity-0'>
            ...
            return <option key={extendedStatus} value={extendedStatus}>
              ...
              <div className='flex items-center gap-2'>
                ...
                <select name='isAdmin-filter' onChange={(e) => {
            ...
            {!isLoading && (<div className='max-w-60'>
                <span className='text-md mr-2 text-black dark:text-white'>page</span>
                <input type='number' min={1} defaultValue={currentPage} max={data?.totalPages} onChange={(e) => {
        ...
        {!!data?.users &&
            data?.users?.length > 0 &&
            data.users.map((user) => (<div key={user.id} className='grid grid-cols-9 gap-4 border-t border-stroke py-4.5 px-4 dark:border-strokedark  md:px-6 '>
              <div className='col-span-3 flex items-center'>
...
//# sourceMappingURL=UsersTable.jsx.map