...
import { validateFile, uploadFileWithProgress, } from './fileUploading';
        ...
        <div className='my-8 border rounded-3xl border-gray-900/10 dark:border-gray-100/10'>
          <div className='space-y-10 my-10 py-8 px-4 mx-auto sm:max-w-lg'>
            <form onSubmit={handleUpload} className='flex flex-col gap-2'>
              <input type='file' id='file-upload' name='file-upload' accept={ALLOWED_FILE_TYPES.join(',')} className='text-gray-600' onChange={() => setUploadError(null)}/>
              <button type='submit' disabled={uploadProgressPercent > 0} className='min-w-[7rem] relative font-medium text-gray-800/90 bg-yellow-50 shadow-md ring-1 ring-inset ring-slate-200 py-2 px-4 rounded-md hover:bg-yellow-100 duration-200 ease-in-out focus:outline-none focus:shadow-none hover:shadow-none disabled:cursor-progress'>
                {uploadProgressPercent > 0 ? (<>
                    <span>Uploading {uploadProgressPercent}%</span>
                    <div role='progressbar' aria-valuenow={uploadProgressPercent} aria-valuemin={0} aria-valuemax={100} className='absolute bottom-0 left-0 h-1 bg-yellow-500 transition-all duration-300 ease-in-out rounded-b-md' style={{ width: `${uploadProgressPercent}%` }}></div>
                  </>) : ('Upload')}
              </button>
            ...
            <div className='space-y-4 col-span-full'>
              ...
              {!!allUserFiles.data && allUserFiles.data.length > 0 && !allUserFiles.isLoading ? (allUserFiles.data.map((file) => (<div key={file.key} className={cn('flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3', {
                'opacity-70': file.key === fileKeyForS3 && isDownloadUrlLoading,
            })}>
                    <p>{file.name}</p>
                    <button onClick={() => setFileKeyForS3(file.key)} disabled={file.key === fileKeyForS3 && isDownloadUrlLoading} className='min-w-[7rem] text-sm text-gray-800/90 bg-purple-50 shadow-md ring-1 ring-inset ring-slate-200 py-1 px-2 rounded-md hover:bg-purple-100 duration-200 ease-in-out focus:outline-none focus:shadow-none hover:shadow-none disabled:cursor-not-allowed'>
                      {file.key === fileKeyForS3 && isDownloadUrlLoading ? 'Loading...' : 'Download'}
                    </button>
                  </div>))) : (<p>No files uploaded yet :(</p>)}
            </div>
          </div>
        </div>
      </div>
