...
export async function handleCheckoutSessionCompleted(session, prismaUserDelegate) {
...
export async function handleInvoicePaid(invoice, prismaUserDelegate) {
...
export async function handlePaymentIntentSucceeded(paymentIntent, prismaUserDelegate) {
    ...
    return updateUserStripePaymentDetails({ userStripeId, numOfCreditsPurchased, datePaid }, prismaUserDelegate);
}
export async function handleCustomerSubscriptionUpdated(subscription, prismaUserDelegate) {
...
export async function handleCustomerSubscriptionDeleted(subscription, prismaUserDelegate) {
    const userStripeId = subscription.customer;
    return updateUserStripePaymentDetails({ userStripeId, subscriptionStatus: SubscriptionStatus.Deleted }, prismaUserDelegate);
}
...
function getPlanIdByPriceId(priceId) {
    const planId = Object.values(PaymentPlanId).find((planId) => paymentPlans[planId].getPaymentProcessorPlanId() === priceId);
    if (!planId) {
        throw new Error(`No plan with Stripe price id ${priceId}`);
    }
    return planId;
}
function getPlanEffectPaymentDetails({ planId, planEffect, }) {
