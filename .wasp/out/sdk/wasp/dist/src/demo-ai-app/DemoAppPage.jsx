...
export default function DemoAppPage() {
    ...
    return (<div className='max-w-3xl mx-auto py-12 px-4 space-y-6'>
      ...
      <div className='h-[60vh] overflow-y-auto rounded-lg border border-gray-200 p-4 bg-white/80 space-y-4 shadow-sm'>
        {messages.map((m, idx) => (<div key={idx} className={cn('whitespace-pre-wrap rounded-lg px-4 py-2 text-sm', m.role === 'user'
      ...
      <div className='flex gap-2'>
        <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder='Sorunuzu yazın...' className='flex-1 rounded-md border border-gray-300 px-3 py-2 focus:outline-none'/>
        <button onClick={handleSend} disabled={isLoading} className='rounded-md bg-yellow-500 px-4 py-2 font-medium text-white hover:bg-yellow-600 disabled:opacity-50'>
          Gönder
        </button>
      </div>
    </div>);
