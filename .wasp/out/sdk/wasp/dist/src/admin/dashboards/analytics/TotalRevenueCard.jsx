...
const TotalRevenueCard = ({ dailyStats, weeklyStats, isLoading }) => {
    ...
    const deltaPercentage = useMemo(() => {
        if (!weeklyStats || weeklyStats.length < 2 || isLoading)
            return;
        if (weeklyStats[1]?.totalRevenue === 0 || weeklyStats[0]?.totalRevenue === 0)
            return 0;
...
//# sourceMappingURL=TotalRevenueCard.jsx.map